...
- `/status`: create a public, self-updating English status panel in the invoking channel. Default command permission: Manage Guild; optional DISCORD_COMMAND_ROLE_ID replaces it with a strict role requirement for all slash commands. One panel per channel; repeated invocation returns its link rather than creating duplicates. Guild-only.
