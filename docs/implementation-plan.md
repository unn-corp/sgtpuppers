...
- Use standard embed fields and three compact faction embeds with emblem images from the community Wardogs Handbook; env-configured custom emoji or image URLs can override them. Show numeric scores without a maximum until a real score cap is available. Accurate proportional score bars can be added when the cap exists; image rendering is unnecessary for this initial version.
