...
- `/status`: create a public, self-updating English status panel in the invoking channel. Default command permission: Manage Guild. One panel per channel; repeated invocation returns its link rather than creating duplicates. Guild-only.
...
- Use standard embed fields and one compact faction-score embed with application-owned emoji sourced from the community Wardogs Handbook, followed by the banner as the final embed; env-configured custom emoji or source image URLs can override the emblems. Show numeric scores without a maximum until a real score cap is available. Accurate proportional score bars can be added when the cap exists; image rendering is unnecessary for this initial version.
